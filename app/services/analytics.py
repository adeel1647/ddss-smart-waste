...
async def get_analytics_overview(session: AsyncSession, organisation_id: int | None = None) -> dict:
    ...
    telemetry_stmt = select(func.avg(Telemetry.fill_level)).select_from(Telemetry).join(Bin, Bin.bin_id == Telemetry.bin_id).where(Telemetry.ts >= today)
    max_fill_stmt = select(func.max(Telemetry.fill_level)).select_from(Telemetry).join(Bin, Bin.bin_id == Telemetry.bin_id).where(Telemetry.ts >= today)
    critical_stmt = select(func.count()).select_from(Alert).join(Bin, Bin.bin_id == Alert.bin_id).where(Alert.created_at >= today, Alert.severity == 'critical')
    open_stmt = select(func.count()).select_from(Alert).join(Bin, Bin.bin_id == Alert.bin_id).where(Alert.status == 'open')
    class_stmt = select(Classification.predicted_class).select_from(Classification).join(Bin, Bin.bin_id == Classification.bin_id).where(Classification.ts >= today)
    ...
    latest_route_distance_km = float((await session.scalar(select(RoutePlan.total_distance_km).order_by(RoutePlan.ts.desc()).limit(1))) or 0.0)
